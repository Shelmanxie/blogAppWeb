    ...
    async def __call__(self, request):
            ...
            if request.method == 'POST':
             # 判断是否村存在Content-Type（媒体格式类型），一般Content-Type包含的值：
                ...
                if not request.content_type:
                    return web.HTTPBadRequest(reason='Missing Content-Type.',text='Missing Content-Type.')   # changed by shelman
                ct = request.content_type.lower()
                if ct.startswith('application/json'):
                    params = await request.json() # 如果请求json数据格式
                    # 是否参数是dict格式，不是的话提示JSON BODY出错
                    if not isinstance(params, dict):
                        return web.HTTPBadRequest(reason='JSON body must be object.',text='JSON body must be object.')  # changed by shelman
                ...
                # POST提交请求的类型
                    ...
                    return web.HTTPBadRequest(text='Unsupported Content-Type: %s' % request.content_type)  # changed by shelman
            if request.method == 'GET':# 调用post方法，注意此处已经使用了装饰器
        ...
        if self._required_kw_args:
            for name in self._required_kw_args:
                if not name in kw:
                    return web.HTTPBadRequest(text='Missing argument: %s' % name)# changed by shelman
...
def add_routes(app, module_name):
    ...
    for attr in dir(mod):
        ...
        # 取能调用的，说明是方法
        if callable(fn):
             # 检测方法里的属性'__method__'和'__route__' ，  这两个属性是我们用装饰器包装的
            method = getattr(fn, '__method__', None)
            path = getattr(fn, '__route__', None)
            if method and path:
